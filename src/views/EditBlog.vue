<template>
  <div class="container">
    ...
    <div class="mainFunction">
      <div @click="updateBlog" class="functionButton">Update Blog</div>
...
export default {
  ...
  async created() {
    ...
    this.currentBlog = await this.$store.state.blogPosts.filter((post) => {
      return post.blogID === this.blogID;
    });
  ...
  methods: {
    ...
    async updateBlog() {
      const dataBase = db.collection("blogs").doc(this.blogID);
      if (this.blogTitle !== "" && this.blogContent !== "") {
        if (this.file) {
          ...
          docRef.put(this.file).on(
            ...
            async () => {
              ...
              await this.$store.dispatch("updatePost", this.blogID);
              this.loading = false;
              this.modalMessage = "Your post have succesfully updated";
        ...
        await this.$store.dispatch("updatePost", this.blogID);
        this.loading = false;
        this.$router.push("/blogs/" + dataBase.id);
        return;
      }
      this.error = true;
